...
def get_totalDomain(inDir,prd):
    """
    Get the total precipitation for a region over time.    
    ...
    --------
    totalDomain: ndarray
     A 3D array containing the total precipitation data, 
     with dimensions (time, y, x).
    """
    meta = get_meta(inDir)
    dtype = np.dtype('f')
    if (meta['prd'] != prd):
        print("Be careful, you are mixing up products!")
    f = open(os.path.join(inDir,"totalroi.bin"), "rb")
    rdata = np.fromfile(f,dtype)
    totalDomain = np.flipud(np.rot90(np.transpose(rdata.reshape([int(meta['z']), int(meta['height']), int(meta['width'])]))))
    totalDomain[totalDomain < 0] = -1.0
    return totalDomain

def get_totalRoi(rectangle,totalDomain):
    """
    Get a subregion of an array representing a region of interest.
    ...
    --------
    totalRoi: ndarray
     An array containing precipitation within the specified region.
    """
    totalRoi = totalDomain[coordy2ind(rectangle[3]):coordy2ind(rectangle[2]), coordx2ind(rectangle[0]):coordx2ind(rectangle[1]),:]
...
def get_POH_domain(dir,rectangle,POHSingleFiles=False):
    ...
    --------
    POH_domain: ndarray
     array containing the POH data for the region
    maxPOHOverTime: ndarray or None
     if POHSingleFiles is True, an array containing the max POH over 
     the region, per 5-min timestep 
    """
...
def get_single_POH(dir,rectangle):
    ...
    --------
    maxPOH: ndarray
     An array containing the max POH over the region, per 5-min timestep 
    """    
...
def get_totalSumRois(totalRoi,prd):
    """
    Get the rainfall depth over the region of interest
    ...
    totalSumRoiS: ndarray
     containing rainfall depth over given time period
